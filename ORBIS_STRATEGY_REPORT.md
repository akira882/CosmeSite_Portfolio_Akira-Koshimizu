...
| **"成分で納得したい"<br>(安全性と根拠の確認)** | **Transparent Safety**<br>(透明性ある安全性) | 「86%」という数値だけでなく、**Safety Specs（無油分・無香料・アレルギーテスト済）**を提示。彼女の検索意図の深層にある「肌トラブルへの恐怖」を先回りして払拭する。 |
| **"失敗したくない"<br>(レビューは3件以上読む)** | **Risk Reversal**<br>(リスク排除) | 「私と同じような人」の声を求めているため、**Reviews**で同年代・同肌質の声を掲載し、**Final CTA**で「無料」を提示して全リスクを肩代わりする。 |

### 🔍 Deep Dive: What is "Search" for Mariko? (佐藤真理子の検索行動の本質)
彼女にとっての「成分検索」とは、単なるオーガニック確認ではありません。
**「私の肌（生活）を壊さないか？」というリスクマネジメント**です。

1.  **Negative Check (ネガティブチェック)**:
    *   「アルコールで赤くならないか？」
    *   「油分でテカって、夕方の会議で恥をかかないか？」
    *   → この不安を消すために **「無油分・無香料・ノンコメドジェニックテスト済」** の表記が必須です。
2.  **Positive Check (ポジティブチェック)**:
    *   「86%の中身は何？」
    *   → 謎の化学物質ではなく、**「植物由来（Botanical）」** であることが、心理的な安心材料（Excuse）になります。

この「守りの検索（Safety）」と「攻めの検索（Efficacy）」の両方を満たすことが、真のSearchハックです。
